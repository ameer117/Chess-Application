...
{
	if( Board[4][0].value == 50 && Board[4][0].color == color)
	{
	...
	else if( Board[4][7].value == 50 && Board[4][7].color == color)
	{
...
int QueenCastle( char color )
{
	...
	if( Board[4][7].value == 50 && Board[4][7].color == color)
	{
...
int main()
{
	...
	int totaltime1, totaltime2, timediff; 
	int totaltimesec, totaltimemin, totaltimehr;
	int i, j, AIorHuman = 0;  
	int continu = 1, game = 1, loop = 1, loop2 = 1;
	int side = 0;
	int mode, move, legal, level;
	char AIcolor1 = 'w';
	char AIcolor2 = 'b';
	char new_or_load[10];
	char mode_char[5];
	...
	while(continu == 1)
	{
	   ...
	   while(game == 1)
	   {
		...
		{    printboard(0);
		     ...
		     printf("Additional Options: Quit Game, Save Game, Undo\n");
		     ...
		     while(legal != 1)
		     {
			   ...
			   if(moveto[1] == 'a' && moveto[2] == 's')   
			   {
				...
				else if( CastQ == 1)
				{
		   	   ...
		   	   legal = LegalMoveChecker( movefrom, moveto, 'w');
			   if( legal == 1)
			   {
		...
		if( (mode < 3 && side == 1) || mode == 1)
		{
		     ...
		     while(legal != 1)
		     {
			   ...
			   legal = LegalMoveChecker(movefrom, moveto, 'b'); 
			   if (legal == 1)
			   {
				    printf("PLEASEWORK LIKE FORREALL");
				    timediff = (int)difftime(timeend, timebegin);
				    if (mode == 2)
				    {
					totaltime1 = totaltime1 + timediff;
